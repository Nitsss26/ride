...
const DRIVER_ID = "680fe3248d614d795224b521";
...
export default function DriverApp() {
    ...
    const reconnectAttempts = useRef(0); // Track reconnection attempts for notificationWs
        ...
        const setupWebSockets = () => {
            ...
            if (isOnline && (!locationWs.current || locationWs.current.readyState === WebSocket.CLOSED)) {
                ...
                locSocket.onopen = () => {
                    ...
                    if (locationWs.current === locSocket) {
                ...
                locSocket.onmessage = (event) => {
                    try {
                        ...
                        console.error("Error parsing location WS message", e);
                    }
                };
            ...
            if (!notificationWs.current || notificationWs.current.readyState === WebSocket.CLOSED) {
                ...
                notifSocket.onopen = () => {
                    ...
                    if (notificationWs.current === notifSocket) {
                        notificationPingInterval = setInterval(() => {
                            if (notificationWs.current && notificationWs.current.readyState === WebSocket.OPEN) {
                                notificationWs.current.send(JSON.stringify({ type: 'ping' }));
                            }
                        }, 30000);
                    }
                };
                ...
                notifSocket.onclose = (event) => {
                    ...
                    if (reconnectAttempts.current < maxReconnectAttempts) {
                        ...
                        setErrorMessage("Failed to reconnect to Notification Service after multiple attempts.");
                        setStatusMessage("Disconnected. Please restart the app.");
                    }
                };
        ...
        switch (data.type) {
            case 'ride_offer':
                if (currentStatus === 'available') {
                    ...
                    sendNotificationResponse('driver_reject', data.rideId, 'Driver is busy');
                }
                break;
            case 'offer_accepted':
            ...
            case 'arrival_confirmed':
                ...
                updateBackendStatus('at_pickup', data.rideId);
                break;
            case 'ride_started':
                setStatusMessage(`Ride ${data.rideId} is in progress.`);
                setCurrentStatus('on_ride');
    ...
    const handleAcceptOffer = async () => {
        ...
        const success = sendNotificationResponse('driver_accept', rideId);
        if (success) {
            setCurrentRide({
                rideId: rideId,
            ...
            setCurrentStatus('en_route_pickup');
            setActiveRideOffer(null);
            setStatusMessage(`Accepted ride ${rideId}. Proceed to pickup.`);
        } else {
            setStatusMessage('Failed to accept ride. Please try again.');
        }
        setIsLoading(false);
    };
    ...
    const handleRejectOffer = () => {
        ...
        const success = sendNotificationResponse('driver_reject', rideId, 'Rejected by driver');
        if (success) {
            setActiveRideOffer(null);
            setStatusMessage(isOnline ? 'Online and ready.' : 'Offline.');
        } else {
            setStatusMessage('Failed to reject ride. Please try again.');
        }
    };
    ...
    setIsLoading(true);
    try {
        // Update driver's status to at_pickup
        await updateBackendStatus('at_pickup', rideId);
        
        // Notify Ride Service to update ride status to driver-arrived
        await apiRequest(`${RIDE_SERVICE_URL}/rides/${rideId}/status`, 'PUT', { status: 'driver-arrived', driverId: DRIVER_ID });
    ...
    const handleCompleteRideCash = async () => {
        ...
        try {
            const response = await apiRequest(`${PAYMENT_SERVICE_URL}/payments/cash`, 'POST', {
                rideId: rideId,
                driverId: DRIVER_ID,
                riderId: "rider_unknown",
                amount: currentRide.estimatedFare?.amount || 10.00,
            ...
            return (
                <View style={styles.statusContainer}>
                    ...
                    {currentStatus === 'on_ride' && (
                        <TouchableOpacity
                            style={[styles.button, isLoading && styles.buttonDisabled]}
                            onPress={handleCompleteRideCash}
                            disabled={isLoading}
                        >
...
const styles = StyleSheet.create({
    ...
    header: {
        ...
        paddingVertical: 10,
        borderBottomWidth: 1,
        borderBottomColor: THEME.muted + '40',
        backgroundColor: THEME.background,
    },
    headerTitle: {
        fontSize: 18,
    ...
    contentOverlay: {
        ...
        left: 0,
        right: 0,
        backgroundColor: THEME.background + 'E9',
    ...
    acceptButton: {
        backgroundColor: THEME.goOnline,
        minWidth: '45%',
        marginHorizontal: 5,
    },
    rejectButton: {
        backgroundColor: THEME.goOffline,
        minWidth: '45%',
        marginHorizontal: 5,
    },
    cancelButton: {
...
// // const DRIVER_ID="680f27e5fd775a644ff82f96";
// // const LOCATION_UPDATE_INTERVAL = 5000; // 5 seconds
...
// const DRIVER_ID = "680fc43d8d614d795224b32e";
// const LOCATION_UPDATE_INTERVAL = 5000; // 5 seconds

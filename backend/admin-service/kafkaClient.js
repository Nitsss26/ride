...
// Kafka publishing helper for admin events
async function publishAdminActionEvent(eventType, eventData) {
  ...
  try {
    ...
    const topic = "admin-events" // Central topic for admin-related events
    const message = {
      key: eventData.adminId ? eventData.adminId.toString() : "system",
    ...
    console.log(`Published ${eventType} event to Kafka topic ${topic}`)
  } catch (error) {
